...
public:
    ...
    Vector3 get_interpolated_pos(GridPos current, GridPos previous, float progress) {
        ...
        bool same_face = Vector3DotProduct(cur_norm, prev_norm) > 0.99f;
        if (same_face) {
            // Reto dentro da mesma face: lerp cartesiano é o caminho geometricamente correto.
            return Vector3Lerp(prev_pos, cur_pos, progress);
        }

        // Virou uma quina (mudou de face): interpola em arco ao redor do centro
        // do cubo em vez de cortar por dentro em linha reta. É isso que dá a
        // sensação de "rolar" pela quina com velocidade constante, em vez do
        // salto/puxão de antes.
        ...
        Vector3 cur_rel = Vector3Subtract(cur_pos, center);
        Vector3 prev_rel = Vector3Subtract(prev_pos, center);
        float cur_len = Vector3Length(cur_rel);
        float prev_len = Vector3Length(prev_rel);

        Vector3 dir = vector3_slerp(Vector3Normalize(prev_rel), Vector3Normalize(cur_rel), progress);
        float len = prev_len + (cur_len - prev_len) * progress;

        return Vector3Add(center, Vector3Scale(dir, len));
    }
...
private:
    ...
    void spawn_food(){
        ...
        float mid { (static_cast<float>(Settings::GRID_SIZE) - 1) / 2.0f };

        m_grid.foreach_cell([&](CubeType t, GridPos p){
            if(t != CubeType::Wall) return;
            candidates.push_back(p);

            // Quão longe do centro da face essa célula está (o eixo fixo da
            // face não conta; olhamos só os dois eixos que variam nela).
            Vector3 normal = surface_normal(p);
            array<float, 3> n { normal.x, normal.y, normal.z };
            float dist_from_face_center {};
            for (int axis {}; axis < 3; axis++) {
                if (std::abs(n[axis]) > 0.5f) continue; // eixo fixo dessa face
                dist_from_face_center = std::max(dist_from_face_center, std::abs(p[axis] - mid));
            }
            // Peso maior perto do centro da face; nunca zero, pra ainda dar pra
            // cair perto da borda de vez em quando.
            double weight { std::max(0.15, static_cast<double>(mid - dist_from_face_center) + 1.0) };
            weights.push_back(weight);
        });
        if(candidates.empty()) terminate();

        discrete_distribution<size_t> dist { weights.begin(), weights.end() };
        m_grid.at(candidates[dist(generator)]) = CubeType::Food;
    }
private:
    Snake m_snake {};
    Grid m_grid {};
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        DrawText(TextFormat("Pontuacao: %d", game.score()), 20, 20, 24, WHITE);
        if (game.has_lost()) {
